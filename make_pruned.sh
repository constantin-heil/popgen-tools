...
docker run --rm -v $PWD:$PWD plink plink \
--bfile $PWD/$WORKPATH/bed \
--indep-pairwise 50 5 0.2 \
--out $PWD/$WORKPATH/prune \
--allow-extra-chr

# Generate pruned dataset
docker run --rm -v $PWD:$PWD plink plink \
--bfile $PWD/$WORKPATH/bed \
--extract $PWD/$WORKPATH/prune.prune.in \
--make-bed \
--out $PWD/$WORKPATH/bed_prune \
--allow-extra-chr
